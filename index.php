...
<div class="form-filters">
    ...
    <form action="<?php echo site_url() ?>/wp-admin/admin-ajax.php" method="POST" class="filter-form" id="filter">
        ...
        if($terms = get_terms('categorie', 'orderby=name')):
            echo '<div class="custom-dropdown"><select name="categoryfilter" class="categorie" selected-option"><option value="">Catégories</option>';
            foreach($terms as $term):
                echo '<option class="filter-option" value="'.$term->term_id.'">'.$term->name.'</option>';
            endforeach;
            echo '</select></div>';
        endif;
        ?>
